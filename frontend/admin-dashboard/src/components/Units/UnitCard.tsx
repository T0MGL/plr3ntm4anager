      ...
      <div className="relative h-48 w-full overflow-hidden bg-slate-100">
        {imageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#f7f7f7] via-white to-[#f2f2f2]">
            <p className="text-sm font-medium tracking-wide text-[#8a8a8a]">{t('unitCard.noImage')}</p>
          </div>
        )}
        <span
          className={`absolute right-3 top-3 rounded-full border px-2.5 py-1 text-xs font-semibold capitalize ${statusClassName}`}
        >
          {status}
        </span>
      </div>
      ...
      <div className="space-y-4 p-5">
        <div className="space-y-2">
          ...
          <p className="min-h-10 text-sm leading-5 text-[#6a6a6a]">
            {description?.trim() ? description : t('unitCard.noDescription')}
          </p>
        </div>

        <div className="flex items-center justify-between">
          <p className="text-base font-semibold text-[#222222]">
            ${nightlyRate}{t('unitCard.perNight')}
          </p>
          {typeof maxGuests === 'number' && (
            <p className="inline-flex items-center gap-1.5 text-xs font-medium text-[#6a6a6a]">
              ...
              {t('unitCard.guests', { count: maxGuests })}
            </p>
          )}
        </div>
